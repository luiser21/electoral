...
for ($i = 2; $i <= $registros; $i++) {
	...
	$ocupacion[$y]=$data->sheets[0]['cells'][$i][5];	
...
for($i=0; $i<$registros-1; $i++){		
	try{		
		...
		if(!empty($puestoreg['ERROR'])){
			...
			//imprimir($puestoreg);
			echo '<br/>';
			echo '[DEPARTAMENTO] = '.$DEPARTAMENTO=trim($puestoreg['DEPARTAMENTO']);
			echo '<br/>';
			echo '[MUNICIPIO] = '.$MUNICIPIO=trim($puestoreg['MUNICIPIO']);
			echo '<br/>';
			echo '[PUESTO] = '.$PUESTO=trim($puestoreg['PUESTO']);
			echo '<br/>';
			echo '[DIRECCION] = '.$DIRECCION=trim($puestoreg['DIRECCION']);
			echo '<br/>';
			echo '[MESA] = '.$MESA=trim($puestoreg['MESA']);
			echo '<br/>';
