<?php
/*
Funciones para envio de datos a IW
*/


function sendReq ($url, $authKey, $idInterface, $idEstado, $idEntradaCaller, $asyncronic, $fechaDiferido, $idCliente,
					 $idEmpresa, $idVenta, $idVentaPadre, $idServicio, $idProducto, $idServicioPadre,
					 $idProductoPadre, $idPromotor, $xmlEncoding, $debug = false) {
	...
	//This is the object to create a service
	$PcspObjCreateService = array(
		'idEntradaCaller' => $idEntradaCaller,
		'idInterface' => $idInterface,
		'idEstado' => $idEstado,
		'asyncronic' => $asyncronic,
		'fechaDiferido' => $fechaDiferido,
		'idCliente' => $idCliente,
		'idEmpresa' => $idEmpresa,
		'idVenta' => $idVenta,
	...
	//This generate the proxy to create a service
	$result = $proxy->Put($authKey, array($PcspObjCreateService),0);
	...
	// SOAP Timeout Handling
	if($result[0]["idError"] == "")
	{
		// SOAP Timeout.
		$result[0]["idError"] == -255;
		$result[0]["errorStr"] = "SOAP Timeout";
		trigger_error("SOAP Timeout");
	}

	return $result;
}

function getDocsisStatus ($url, $authKey, $idEmpresaCRM, $idServicio, $idVenta, $idProducto, $xmlEncoding, $debug = false)
{
	// Create the client instance
	$client = new nusoap_client($url, true);
	...
	//This generate the proxy to create a service
	$result = $proxy->GetDocsisStatus($authKey, $idEmpresaCRM, $idServicio, $idVenta, $idProducto, $xmlEncoding);
	...
	if($result["idError"] == "")
	{
		// SOAP Timeout.
		$result["idError"] == -255;
		$result["errorStr"] = "SOAP Timeout";
		trigger_error("SOAP Timeout");
	}
	return $result;
}

function getReport ($url, $authKey, $idEmpresaCRM, $idClienteCRM, $cantRecords, $showProducts, $debug = false)
{
//	echo date('Y-m-d H:i:s')."function getReport - nusoap IW<br>";
	// Create the client instance
	$client = new nusoap_client($url, true);
	...
	$proxy = $client->getProxy();
	// Call the SOAP method
//	echo date('Y-m-d H:i:s')."function getProxy - nusoap IW<br>";


	//This generate the proxy to create a service
	$result = $proxy->GetReport($authKey, $idEmpresaCRM, $idClienteCRM, $cantRecords, $showProducts);
...
function getReport2 ($url, $authKey, $showProducts, $debug = false)
{
	// Create the client instance
	$client = new nusoap_client($url, true);
	...
	//This generate the proxy to create a service
	$result = $proxy->GetCustomReport($authKey,$showProducts);
//	$result = $proxy->GetReport($authKey, $idEmpresaCRM, $idClienteCRM, $cantRecords, $showProducts);
...
function getActivity($url, $authKey, $idInterface="", $idEstado="", $idEntradaCaller="", $asyncronic="", $fechaDiferido="", $idCliente="",
					 $idEmpresa="", $idVenta="", $idVentaPadre="", $idServicio="", $idProducto="", $idServicioPadre="",
					 $idProductoPadre="", $idPromotor="", $xmlEncoding="", $debug=false)
{
	// Create the client instance
	...
	//This generate the proxy to create a service
	$result = $proxy->GetActivity($authKey, $xmlEncoding);
...
class soap_transport_http extends nusoap_base {
	...
	*/
	...
	function connect($connection_timeout=0,$response_timeout=240){
...
//curl_setopt($this->ch, CURLOPT_CAINFO, 'f:\php-4.3.2-win32\extensions\curl-ca-bundle.crt');
	...
	* @access   public
	*/
	function send($data, $timeout=0, $response_timeout=240) {
	...
	* @access   public
	*/
	function sendHTTPS($data, $timeout=0, $response_timeout=240) {
...
class wsdl extends nusoap_base {
     ...
     * @access public
     */
    function wsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,$response_timeout=240){
        $this->wsdl = $wsdl;
        	...
        	while ($imported > 0) {
        		...
        		foreach ($this->schemas as $ns => $list) {
        			foreach ($list as $xs) {
			            ...
			            foreach ($xs->imports as $ns1 => $list2) {
			                for ($ii = 0; $ii < count($list2); $ii++) {
			                	if (! $list2[$ii]['loaded']) {
			                		$this->schemas[$ns]->imports[$ns1][$ii]['loaded'] = true;
...
/**
*
* nusoap_client higher level class for easy usage.
*
* usage:
*
* // instantiate client with server info
* $soapclient = new nusoap_client( string path [ ,boolean wsdl] );
*
...
class nusoap_client extends nusoap_base  {
	...
	var $timeout = 60;				// HTTP connection timeout
	var $response_timeout = 240;		// HTTP response timeout
	var $endpointType = '';
	var $persistentConnection = false;
    ...
    var $decode_utf8 = true;
    var $TargetNamespace = 'http://testuri.org';
	...
	* @access   public
	*/
	function nusoap_client($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 60, $response_timeout=240){
	...
	*/
	function call($operation,$params=array(),$namespace='',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			//$namespace = isset($opData['input']['namespace']) ? $opData['input']['namespace'] :	($namespace != '' ? $namespace : 'urn:OutboundWebService'); // Lacides
			$namespace = isset($opData['input']['namespace']) ? $opData['input']['namespace'] :	($namespace != '' ? $namespace : $this->TargetNamespace);
			$style = $opData['style'];
			...
			// no WSDL
            if($namespace == ''){
            	//$namespace = 'urn:OutboundWebService'; // Lacides
	...
	* @access   private
	*/
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=240) {
		// detect transport
		switch(true){
			// http(s)
	...
	function getProxy(){
		...
		foreach($this->operations as $operation => $opData){
			if($operation != ''){
				...
				//$opData['namespace'] = !isset($opData['namespace']) ? 'urn:OutboundWebService' : $opData['namespace']; // Lacides
				$opData['namespace'] = !isset($opData['namespace']) ? $this->TargetNamespace : $opData['namespace'];
