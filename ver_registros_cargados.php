...
UNION
SELECT 10 as ID,CONCAT(ROUND((TRANSFERIR/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,TRANSFERIR,'  - La informacion se encuentra en actualizacion' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
...
SELECT 10 as ID,CONCAT(ROUND((count(CEDULA)/(SELECT (APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS) from upload_file  WHERE ESTADO='A' AND id=".$_GET["id"]."))*100,0),'%') AS PORCENTAJE,count(CEDULA),'Para Reprocesar' AS DETALLE from tmp_miembros WHERE PUESTO<>'Cedula ya existe'AND IDFILE=".$_GET["id"]."";		
			...
			$sql="SELECT 14 as ID,'100%' AS PORCENTAJE,(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS) as MUERTE,'Total Registros Procesados' AS DETALLE from upload_file  WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 15 as ID,CONCAT(ROUND((APTOSVOTAR/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,APTOSVOTAR,'1.APTOS PARA VOTAR' AS DETALLE from upload_file  WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION ";
			if($_SESSION["tipocandidato"]=='ALCALDIA'){
			$sql.="
SELECT 16 as ID,CONCAT(ROUND((DATOSVALIDOOS/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,DATOSVALIDOOS,'  - Inscritos en el Municipio' AS DETALLE from upload_file  WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 17 as ID,CONCAT(ROUND((DIFERENTEMUNICIPIO/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,DIFERENTEMUNICIPIO,'  - No Inscrito en el Municipo ' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."
UNION ";
			}
			$sql.="
SELECT 18 as ID,CONCAT(ROUND(((NOAPTOSVOTAR)/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,(NOAPTOSVOTAR),'2.NO APTOS PARA VOTAR SEGUN REGISTRADURIA NACIONAL' AS DETALLE from upload_file  WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 4 as ID,CONCAT(ROUND((DEBEINSCRIBIRSE/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,DEBEINSCRIBIRSE,'  - No se encuentra en el censo para las próximas elecciones' AS DETALLE from upload_file  WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
...
UNION
SELECT 8 as ID,CONCAT(ROUND((DOBLECEDULACION/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,DOBLECEDULACION,'  - Cancelada por Doble Cedulacion' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 5 as ID,CONCAT(ROUND((TRASHUMANCIA/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,TRASHUMANCIA,'  - Baja por Trashumancia' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 6 as ID,CONCAT(ROUND((INHUMACION/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,INHUMACION,'  - Baja por Inhumacion o Necrodactilia Positiva' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 10 as ID,CONCAT(ROUND((TRANSFERIR/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,TRANSFERIR,'  - La informacion se encuentra en actualizacion' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 7 as ID,CONCAT(ROUND((VIGENTE/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,VIGENTE,'  - Vigente' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 12 as ID,CONCAT(ROUND((INCORRECTO/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,INCORRECTO,'  - Numero de documento Incorrecto' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 3 as ID,CONCAT(ROUND((PENDIENTE/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,PENDIENTE,'  - Pendiente por Solicitud en proceso' AS DETALLE from upload_file WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
SELECT 18 as ID,CONCAT(ROUND(((DATOSINVALIDOS)/(APTOSVOTAR+NOAPTOSVOTAR+DATOSINVALIDOS))*100,2),'%') AS PORCENTAJE,(DATOSINVALIDOS),'2.NO APTOS PARA VOTAR POR ERROR PROCESAMIENTO' AS DETALLE from upload_file  WHERE ESTADO='A' AND id=".$_GET["id"]."		
UNION
...
//imprimir();			
	...
	if(!empty($_GET["idtipo"]) && !empty($_GET["miembrosver"]))
	{	
//echo 'hola';
		$sql="SELECT
				miembros.ID,
...
//imprimir();			
			for($i=0; $i<count($miembros);$i++){
				$row[$i]['ID']=$miembros[$i]['ID'];
				$row[$i]['NOMBRES']=$miembros[$i]['NOMBRES'];
				$row[$i]['CEDULA']=$miembros[$i]['CEDULA'];
				$row[$i]['LIDER']=$miembros[$i]['LIDER'];
			}	
			//Return result to jTable
			$jTableResult = array();
			$jTableResult['Result'] = "OK";
